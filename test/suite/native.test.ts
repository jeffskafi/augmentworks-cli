...
describe("real hosted customer-suite producer contract", () => {
  it("pins the byte-for-byte upstream schema and fixtures and accepts producer sources", async () => {
    const lock = JSON.parse(await readFile(new URL("contracts/aw-suite-v1.lock.json", root), "utf8"));
    const schema = await readFile(new URL("contracts/aw-feature-v1.producer.schema.json", root));
    const fixtures = await readFile(new URL("contracts/aw-feature-v1.producer.fixtures.json", root));
  ...
  it("maps the actual website owned-fixture file to native IDs, references, and required criteria", async () => {
    const loaded = await loadCustomerSuiteFile(new URL("test/fixtures/native-suite-producer/owned-widget.suite.yaml", root).pathname);
  ...
  it("rejects unsupported observations instead of discarding their assertions", async () => {
    const loaded = await loadCustomerSuiteFile(new URL("examples/customer-suites/returns-14-day.yaml", root).pathname);
  ...
  it("rejects authoring values outside native limits without truncating them", async () => {
    const loaded = await loadCustomerSuiteFile(new URL("examples/customer-suites/faq-non-commerce.yaml", root).pathname);

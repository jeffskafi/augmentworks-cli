...
This `@augmentworks/cli@0.3.6` package includes `--assessment`. `init` writes
...
```bash
npx --yes @augmentworks/cli@0.3.6 recover
npx --yes @augmentworks/cli@0.3.6 recover --json
```
...
`usage` is in this 0.3.6 package. Run `node dist/index.js usage` from a clone
or `npx --yes @augmentworks/cli@0.3.6 usage` after install. The command uses the existing connector
...
`demo` is in this `0.3.6` package. From a clone, run `node dist/index.js demo`. The demo ignores project

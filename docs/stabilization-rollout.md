...
| This source CLI + stabilized server | Full inspect/retire/resume/cancel once `POST /v1/relay/run-intents:reconcile` (`aw-run-intent-reconcile/0.1`) is deployed. |
| This source CLI + old server | Create/resume within the replay window still works. Reconcile 404/405/501 becomes `RECOVERY_UNSUPPORTED`. Pending journals are not cleared. |
...
3. Publish a new CLI package through the existing trusted-publishing workflow
   when authorized. Do not republish `0.2.0`.
4. Update the website CLI pin and recovery copy only to that published version.
5. `npx --yes @augmentworks/cli@<published> recover --help` must list

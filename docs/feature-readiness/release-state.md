...
| Website pin | Main repository discovery snapshot | Adopted only after independent review of **this** patch. This CLI does not self-adopt. Independently inspected 0.3.6 is the current published pin ready for website adoption. Independently inspected 0.3.5 remains historical; it omits suite-selection `capabilities`. Source 0.3.7 is not npm-published until the protected `v0.3.7` release. Do not fetch `latest`. |
...
Independent inspection of `@augmentworks/cli@0.3.6` remains
`lastIndependentlyInspected` (the prior tarball packaged `LAST_VERIFIED_*`
points at). `thisPackageIdentity` for 0.3.7 stays
`pending-protected-publish` until the trusted npm workflow runs.

Do not overwrite or relabel existing npm versions, including 0.3.6 and 0.3.5.

...
YAML before starting an assessment. Source 0.3.3 can preview the sanitized
...
published npm tarball. `npx @augmentworks/cli@0.3.2` cannot start this
...
`node dist/index.js init --starter response-only` on source 0.3.3.
Published 0.3.2 does not include `probe` or starter generation.
